...
function formatLength(lengthM: number | null, complete: boolean) {
  if (lengthM === null) return "Belum lengkap";
  const value = lengthM >= 1000
    ? `${new Intl.NumberFormat("id-ID", { maximumFractionDigits: 2 }).format(lengthM / 1000)} km`
    : `${new Intl.NumberFormat("id-ID").format(lengthM)} m`;
        ...
        <div key={`${step.urutan}-${step.jenis}-${step.label}`} className={`noc-trace-step is-${step.jenis.toLowerCase()}`}>
          ...
          <div className="noc-trace-step-copy">
            ...
            <div className="noc-trace-step-meta">
              ...
              {step.jenis === "CORE" && <span>{formatLength(typeof step.detail.panjangM === "number" ? step.detail.panjangM : null, true)}</span>}
...
export function TracePanel({ source, focus = "jalur" }: { source: { kind: "otbPort" | "odpPort" | "core"; id: string; label: string; end?: "A" | "B" } | null; focus?: "jalur" | "core" }) {
  ...
  return (
    <div className={`noc-trace-view is-${focus}`}>
      ...
      <div className="noc-trace-overview"><NocMetric label="Cabang" value={data.ringkas.total} note={`${data.ringkas.lengkap} lengkap · ${data.ringkas.bermasalah} bermasalah`} /><NocMetric label="Jalur aktif" value={branch.ringkas.hop} note="hop" /><NocMetric label="Panjang" value={formatLength(branch.ringkas.panjangM, branch.ringkas.panjangLengkap)} note="meter / estimasi server" /><NocMetric label="Estimasi loss" value={`${branch.ringkas.estimasiLossDb} dB`} note={`${branch.ringkas.sambunganPakaiModel} sambungan memakai model`} /></div>
      {data.jalur.length > 1 && <div className="noc-trace-branches" role="tablist" aria-label="Cabang jalur trace">{data.jalur.map((path, index) => <button key={`${path.status}-${index}`} type="button" role="tab" aria-selected={index === branchIndex} className={`noc-trace-branch ${index === branchIndex ? "is-selected" : ""}`} onClick={() => setBranchIndex(index)}><strong>Jalur {index + 1}</strong><NocStatus label={path.status} tone={pathTone(path.status)} /></button>)}</div>}
      <div className="noc-trace-status"><NocStatus label={branch.status} tone={pathTone(branch.status)} />{branch.diagnosis && <p>{branch.diagnosis}</p>}{data.ringkas.terpotong && <p>Jumlah cabang dipotong oleh batas aman mesin trace.</p>}</div>
